    ...
    return (
      <div className="space-y-3">
        {[0, 1, 2].map((index) => (
          <div
            key={index}
            className="glass-panel w-full border border-dashed h-28 border-muted-foreground/30"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="mt-4 space-y-4">
      {claudeDesktopStatusMessages.length > 0 && (
        <div className="glass-panel rounded-2xl border border-amber-300/45 bg-amber-100/25 px-4 py-3 text-sm text-amber-900 dark:text-amber-200">
      ...
      {filteredProviders.length === 0 ? (
        <div className="glass-panel px-6 py-8 text-sm text-center border border-dashed text-muted-foreground">
