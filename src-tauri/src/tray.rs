...
const TRAY_POPOVER_WIDTH: f64 = 368.0;
const TRAY_POPOVER_HEIGHT: f64 = 540.0;

fn tray_section_icon(app_type: &AppType) -> &'static str {
    match app_type {
        AppType::Claude => "🟣",
        AppType::Codex => "🤖",
        AppType::Gemini => "💎",
...
fn provider_menu_label(provider_name: &str, is_current: bool, is_official_blocked: bool) -> String {
    let status_icon = if is_current { "✅" } else { "▫️" };
    let blocked_suffix = if is_official_blocked { " ⛔" } else { "" };
    format!("{status_icon} {provider_name}{blocked_suffix}")
}
...
pub fn create_tray_menu(
    ...
    for section in TRAY_SECTIONS.iter() {
        ...
        if providers.is_empty() {
            ...
            let label = format!(
                "{} {} {}",
                tray_section_icon(&section.app_type),
            ...
            for (id, provider) in sort_providers(&providers) {
                ...
                let label = provider_menu_label(&provider.name, is_current, is_official_blocked);
...
fn ensure_tray_popover(app: &tauri::AppHandle) -> tauri::Result<tauri::WebviewWindow> {
...
fn clamp_popover_position(
    ...
    let width = TRAY_POPOVER_WIDTH * scale;
    let height = TRAY_POPOVER_HEIGHT * scale;
    let margin = 12.0 * scale;

    let min_x = origin.x as f64 + margin;
    let max_x = origin.x as f64 + size.width as f64 - width - margin;
    let min_y = origin.y as f64 + margin;
    let max_y = origin.y as f64 + size.height as f64 - height - margin;

    Ok(PhysicalPosition::new(
        x.clamp(min_x, max_x.max(min_x)).round() as i32,
        y.clamp(min_y, max_y.max(min_y)).round() as i32,
    ))
}

pub fn show_tray_popover_at(app: &tauri::AppHandle, position: PhysicalPosition<f64>) {
    match ensure_tray_popover(app) {
        Ok(window) => {
            let target_x = position.x - TRAY_POPOVER_WIDTH - 12.0;
            let target_y = position.y - TRAY_POPOVER_HEIGHT - 12.0;
            match clamp_popover_position(app, target_x, target_y) {
                Ok(pos) => {
                    let _ = window.set_position(Position::Physical(pos));
                }
                Err(e) => log::warn!("[Tray] 计算托盘弹窗位置失败: {e}"),
            }
            let _ = window.show();
            let _ = window.set_focus();
        }
        Err(e) => log::error!("[Tray] 创建托盘弹窗失败: {e}"),
    }
}

pub fn toggle_tray_popover(app: &tauri::AppHandle, position: PhysicalPosition<f64>) {
    if let Some(window) = app.get_webview_window(TRAY_POPOVER_LABEL) {
    ...
    show_tray_popover_at(app, position);
}
