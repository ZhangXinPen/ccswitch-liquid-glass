...
///
/// GitHub Push Protection 会拦截硬编码 OAuth client secret，即使该值来自公开客户端。
/// 这里改为运行时读取，避免把凭据写入仓库历史。
...
async fn refresh_gemini_token(refresh_token: &str) -> Option<String> {
    let Some((client_id, client_secret)) = gemini_oauth_client_credentials() else {
        log::warn!("Gemini OAuth client credentials are not configured");
        return None;
    };
